...
export function Categories() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showToast(
        `Category "${categoryToDelete.name}" has been deleted.`,
        "success",
      );
    } catch (err: any) {
      showToast(
            ...
            <table className="w-full min-w-[600px]">
              ...
              <tbody className="divide-y divide-[#E4E4E7]/60">
                {paginatedItems.map((cat) => (
                  ...
                  >
                    <td className="px-6 py-4 text-[14px] text-[#4B5563] font-mono font-bold">
                      {formatCategoryCode(cat.id)}
                    </td>
                    <td className="px-6 py-4 text-[14px] text-[#4B5563] font-medium">
                      {cat.name}
                    </td>
                  ...
                  <tr>
                    ...
                    >
                      {searchQuery || statusFilter.length > 0
                        ? "Tidak ada kategori hasil filter."
                        : "No categories found."}
                    </td>
                  </tr>
                )}
          ...
          {totalPages > 0 && (
            <div className="bg-[#FAFAFA] border-t border-[#E4E4E7] px-5 py-3 flex items-center justify-between">
              <div className="text-[10px] font-mono text-[#71717A]">
                ...
                of{" "}
                <span className="font-bold text-[#18181B]">{totalItems}</span>{" "}
                entries
              </div>
