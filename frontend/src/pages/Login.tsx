...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const sqlInjectionPattern =
      /('|;|--|\b(SELECT|UNION|DROP|DELETE|UPDATE|INSERT)\b)/i;
    if (sqlInjectionPattern.test(cleanEmail)) {
      setError(
        "We noticed some unusual characters in your email. Please use standard text.",
      );
      return;
    }
    ...
    try {
      ...
      setError(
        err?.response?.data?.detail ||
          "We couldn't sign you in. Please check if your email and password are correct.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div
      className="min-h-screen flex items-center justify-center p-4 bg-[#F4F4F5] animate-in fade-in duration-700"
    ...
    >
      ...
      <div className="w-full max-w-[1000px] min-h-[550px] flex flex-col md:flex-row rounded-2xl overflow-hidden shadow-xl bg-[#FFFFFF] border border-[#E4E4E7] z-10 relative">
        ...
        <div className="md:w-5/12 p-10 md:p-14 bg-linear-to-br from-[#18181B] to-[#27272A] flex flex-col justify-center border-r border-[#E4E4E7]/50 relative overflow-hidden">
          ...
          <div className="flex flex-col relative z-10">
            <h1 className="text-3xl md:text-4xl font-extrabold mb-3 tracking-tight whitespace-nowrap">
              <span className="text-[#DC2626]">Inventaris</span>{" "}
              <span className="text-white">Siber</span>
            </h1>
            <p className="text-[#A1A1AA] text-[10px] font-medium tracking-[0.2em] normal-case mt-1 whitespace-nowrap">
        ...
        <div className="md:w-7/12 p-10 md:p-14 flex flex-col justify-center bg-[#FFFFFF] relative">
          <h2 className="text-2xl font-bold text-[#18181B] mb-8 tracking-tight">
            Sign In Staff
          </h2>
          ...
          >
            ...
            <div className="space-y-4">
              {/* Email Input */}
              <div>
                <label className="text-sm font-medium text-[#52525B] block mb-2">
                  Email
                </label>
                <div className="relative group">
                  <Mail
                    size={14}
                    className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition-colors ${emailFocused ? "text-[#DC2626]" : "text-[#71717A]"}`}
                  />
                  <input
                    type="email"
                    required
              ...
              <div>
                ...
                <div className="relative group">
                  <Lock
                    size={14}
                    className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition-colors ${passwordFocused ? "text-[#DC2626]" : "text-[#71717A]"}`}
                  />
                  <input
